...
//$where = ['client_id' => $surveyor->userid];
$where = [];
array_push($where, 'AND client_id=' . $this->ci->db->escape_str($client_id));
array_push($where, 'AND client_type='. '"surveyor"');


//$where = hooks()->apply_filters('surveyors_staff_table_sql_where', []);
...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == 'last_login') {
            ...
            if($has_pengguna_edit){
                ...
                $_data .= ' <a href="' . admin_url('surveyors/staff/member/' . $aRow['staffid']) . '">' . $aRow['firstname'] . ' ' . $aRow['lastname'] . '</a>';

                $_data .= '<div class="row-options">';
                $_data .= '<a href="' . admin_url('surveyors/staff/member/' . $aRow['staffid']) . '">' . _l('view') . '</a>';
    ...
    $row = hooks()->apply_filters('surveyors_staff_table_row', $row, $aRow);
